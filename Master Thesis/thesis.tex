...
In Al-Farooque Shubhoe's article "Top 10 ways to optimize data access in SQL Server"\cite{Shubhoe09} he explains how to measure if index fragmentation has occurred. By executing the following script, index fragmentation is analyzed on every table in the database and presented in a table with an internal and external fragmentation value. 
...
According to Shubhoe, only tables with an internal fragmentation value of less than 75 and/or an external fragmentation value of more than 10 should be considered as fragmented, which this code takes into account.

\subsubsection*{Reorganize vs Rebuild}\mbox{}\\
Both rebuilding and reorganizing are built-in operations in SQL-Server 2014.
